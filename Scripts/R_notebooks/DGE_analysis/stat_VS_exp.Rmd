...
samples <- samples[(samples$phase == 'exp' & samples$treat == 'x') | (samples$phase == 'stat' & samples$treat == 'x'), ] # select samples
...
EnhancedVolcano(res.df, 
    ...
    x = 'log2FoldChange', 
    y = 'padj',  
    title = 'Stationary VS exponentional growth phase', 
...
```
```{r}
ggsave('/scratch/okhtienk/_stat_vs_exp_Volcano_plot.pdf', height = 10, width = 10)
```
...
pdf(file='/scratch/okhtienk/stat_vs_exp_complex_heatmap.pdf')
...
write.csv(res_df, file = "/scratch/okhtienk/stat_vs_exp_DEG_results.csv")
```
...
```{r}
DEG_results <-  read.csv("/scratch/okhtienk/stat_vs_exp_DEG_results.csv", header = TRUE, fill=TRUE)
...
```{r}
deg <- DEG_results[(DEG_results$padj<0.05) & (DEG_results$log2FoldChange > 0), ] # up-regulated genes
N <- dim(deg)[1]
